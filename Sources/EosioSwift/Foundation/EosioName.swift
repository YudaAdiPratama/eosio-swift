...
public struct EosioName: Codable, CustomStringConvertible, Equatable, Hashable {
    ...
    /// Init with a String.
    ///
    /// - Parameter name: An VEXCHAIN name as a string.
    /// - Throws: If not a valid VEXCHAIN name.
    public init(_ name: String) throws {
        guard name.isValidEosioName else {
            throw EosioError(.eosioNameError, reason: "\(name) is not a valid vex name.")
        }
        self.string = name
    }
    ...
    public init(from decoder: Decoder) throws {
        ...
        guard string.isValidEosioName else {
            throw EosioError(.eosioNameError, reason: "\(string) is not a valid vex name.")
        }
        self.string = string
    }

    /// Encode the VEXCHAIN name to String.
    ///
...
public extension String {
    ...
    /// - Returns: `true` if a string is a valid VEXCHAIN name.
